...
def main():
    ...
    dataset = dataset.map(lambda x: {"doc_type": dataset["train"].features["doc_typeIPS"].int2str(x["doc_typeIPS"])})

    dataset = dataset.remove_columns(["doc_typeIPS"])
    ...
    dataset.save_to_disk(Path(__file__).parent.parent.parent / "data" / "raw")
